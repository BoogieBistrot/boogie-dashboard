...
exports.handler = async (event) => {
  ...
  try {
    ...
    // — Last minute (prenotato stesso giorno o giorno prima) —
    ...
    // — Media coperti per giorno (solo giorni attivi) —
    ...
    const msg = `Statistiche ${settimana} salvate (${totPrenotazioni} prenotazioni, ${totPersone} persone)`
    console.log(msg)
    return {
      statusCode: 200,
      headers: { 'Access-Control-Allow-Origin': '*' },
      body: JSON.stringify({ success: true, message: msg, settimana, totPrenotazioni, totPersone }),
    }
