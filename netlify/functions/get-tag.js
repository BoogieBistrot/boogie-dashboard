...
exports.handler = async (event) => {
  ...
  try {
    const res = await fetch(
      `https://api.airtable.com/v0/${AIRTABLE_BASE_ID}/Tag?sort[0][field]=Nome&sort[0][direction]=asc`,
      { headers: { 'Authorization': `Bearer ${AIRTABLE_TOKEN}` } }
    );
    if (!res.ok) return { statusCode: 500, headers, body: JSON.stringify({ success: false }) };

    const json = await res.json();
    const tag = (json.records || []).map(r => ({ id: r.id, nome: r.fields['Nome'] || '' })).filter(t => t.nome);
